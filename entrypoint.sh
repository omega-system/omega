...
php -r 'for ($i = 10; $i--; ) try { (new PDO("mysql:host=$argv[1];port=$argv[2]", $argv[3], $argv[4]))->exec("CREATE DATABASE IF NOT EXISTS \`$argv[5]\`"); exit(0); } catch (Exception $e) { printf("warning: " . $e); sleep(10); } exit(1);' -- $OMEGA_DB_HOST $OMEGA_DB_PORT $OMEGA_DB_USER $OMEGA_DB_PASSWORD $OMEGA_DB_NAME || exit 1
